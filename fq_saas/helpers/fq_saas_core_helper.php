...
require(__DIR__ . '/fq_saas_php8_polyfill_helper.php');
require(__DIR__ . '/fq_saas_storage_helper.php');
 ...
 * @return void
 */
function fq_saas_init()
{
    try {
        if (isset($_SERVER['REQUEST_URI'])) {

            $request_uri = $_SERVER['REQUEST_URI'];
            $host  = fq_saas_http_host($_SERVER);

            // Can identify tenant by url segment and or host (subdomain or cname/custom domain)
            $tenant_info = fq_saas_get_tenant_info_by_http($request_uri, $host);
            $options = fq_saas_get_options(
                [
                    'fq_saas_enable_client_bridge',
                    'fq_saas_enable_cross_domain_bridge',
                    'fq_saas_enable_instance_switch',
                    'fq_saas_client_bridge_account_menu_position',
                    FQ_SAAS_GLOBAL_ACTIVE_MODULES_OPTION_KEY,
                    'fq_saas_masked_settings_pages',
                    'fq_saas_trial_expire_page_url',
                    'fq_saas_enable_single_package_mode',
                    'fq_saas_require_invoice_payment_status',
                    'fq_saas_disable_cron_job_setup_tab',
                    'show_subscriptions_in_customers_area',
                    'fq_saas_trial_notification_day',
                    'fq_saas_enable_tenant_admin_modules_page',
                    // Demo instance metadata exposed to tenants so they can render
                    // a heads-up notice before the scheduled demo reset (0.3.8).
                    'fq_saas_demo_instance',
                    'fq_saas_demo_instance_reset_hour',
                    'fq_saas_demo_instance_last_reset_time'
                ],
                true
            );
            ...
            if ($tenant_info) {
                ...
                $field =  $tenancy_access_mode == FQ_SAAS_TENANT_MODE_DOMAIN ? 'custom_domain' : 'slug'; // path and subdomain mode use slug for search
                ...
                $_base_url = fq_saas_url_origin($_SERVER) . '/';
                ...
                if (!$tenant_id)
                    fq_saas_show_tenant_error("Invalid Tenant", "We could not find the requested instance.", 404);

                // Check if tenant exists
                $tenant = fq_saas_search_tenant_by_field($field, $tenant_id);
                if (!$tenant) {
                    fq_saas_show_tenant_error("Invalid Tenant", "The requested tenant does not exist.", 404);
                }
                ...
                $package_invoice = fq_saas_get_client_package_invoice($tenant->clientid, ['include_cancelled' => true]);
                if ($package_invoice) {
                ...
                $GLOBALS[FQ_SAAS_MODULE_NAME . '_tenant'] = $tenant;
                ...
                if (!empty($tenant_path_id)) {
                    ...
                    $tenant_upload_folder  = $tenant_path_id . '/' . FQ_SAAS_UPLOAD_BASE_DIR;
                    ...
                    if ($request_uri_updated) {
                        if (empty($_POST)) {
                            $url = fq_saas_url_origin($_SERVER, true, true);
                            header("Location: $url");
                            exit;
                        }
                    }
                    ...
                    if (stripos($request_uri, ".") && stripos($request_uri, "://") === false) {
                        ...
                        if (
                            ...
                            $url = str_ireplace("/$tenant_path_id", '', fq_saas_url_origin($_SERVER, true, true));
                            header("Location: $url");
                            exit;
                        }
                    }
                ...
                define('FQ_SAAS_TENANT_SLUG', $tenant->slug);
                define('FQ_SAAS_TENANT_BASE_URL', $tenant_base_url);
            ...
            if (!$tenant_info && ($options['fq_saas_enable_cross_domain_bridge'] ?? '0') == '1') {

                $is_admin = str_starts_with(ltrim($request_uri, '/'), 'admin') !== false;
                $is_in_local = fq_saas_host_is_local(fq_saas_get_saas_default_host(), true);
        ...
        fq_saas_show_tenant_error("Initialization Error", $th->getMessage(), 500);
    }

    // Define APP_BASE_URL based on the tenant's base URL, fallback to the default base URL if not available
    define('APP_BASE_URL', defined('FQ_SAAS_TENANT_BASE_URL') ? FQ_SAAS_TENANT_BASE_URL : APP_BASE_URL_DEFAULT);
}
 ...
 */
function fq_saas_get_tenant_info_by_http($request_uri, $host)
{
    ...
    $tenant_info = false;
    $mode = FQ_SAAS_TENANT_MODE_DOMAIN;

    // Try by subdomain or domain first before url match
    if (!empty($host) && !fq_saas_host_is_local($host)) {
        $tenant_info = fq_saas_get_tenant_info_by_host($host);
        if (!empty($tenant_info['slug']))
            $mode = FQ_SAAS_TENANT_MODE_SUBDOMAIN;
    }

    if (!$tenant_info) {
        // Get tenant information from request URI
        $tenant_info = fq_saas_get_tenant_info_by_request_uri($request_uri);
        if ($tenant_info)
            $mode = FQ_SAAS_TENANT_MODE_PATH;
    }
 ...
 */
function fq_saas_get_tenant_info_by_request_uri($request_uri)
{
    $saas_url_marker = '/' . FQ_SAAS_ROUTE_ID;
    ...
    if ($saas_url_id_pos !== false) {
        ...
        $base_url_path = parse_url(fq_saas_default_base_url());
 ...
 */
function fq_saas_get_tenant_info_by_host($http_host)
{
    // Validate input
 ...
 * @return string
 */
function fq_saas_get_saas_default_host()
{
    return parse_url(fq_saas_default_base_url(), PHP_URL_HOST);
}
 ...
 * @return string
 */
function fq_saas_get_saas_alternative_host()
{
    return defined('FQ_SAAS_ALTERNATIVE_HOST') ? FQ_SAAS_ALTERNATIVE_HOST : '';
}
 ...
 * @return string
 */
function fq_saas_default_base_url($path = '')
{
 ...
 * @return bool
 */
function fq_saas_host_is_local($host, $strict = false)
{
 ...
 */
function fq_saas_search_tenant_by_field($field, $value, $permit_cleaning = true)
{
    $value_cleaned = $value;

    if ($field == 'slug' && $permit_cleaning) {
        $value_cleaned = fq_saas_clean_slug($value);
    }

    $tenant_table = fq_saas_table('companies');
    $query = "SELECT `slug`, `name`, `dsn`, `clientid`, `metadata`, `status` FROM `$tenant_table` WHERE `$field` = :value";
    $parameters = [':value' => $value_cleaned];

    $tenant_info = fq_saas_raw_query_row($query, [], true, true, $parameters);
    if ($tenant_info) {
        return $tenant_info;
    }
    ...
    if ($permit_cleaning && $field == 'slug' && $value_cleaned != $value) {
        return fq_saas_search_tenant_by_field($field, $value, false);
    }
 ...
 * 
 * Use Fq_saas_model::get_company_invoice() for comprehensive details
 * 
 ...
 */
function fq_saas_get_client_package_invoice($clientid, $options = [])
{
    $clientid = (int)$clientid;
    $dbprefix = fq_saas_master_db_prefix();
    $invoice_table = $dbprefix . 'invoices';
    $package_table = fq_saas_table('packages');
    ...
    $package_invoice = fq_saas_raw_query_row($q, [], true, true, $parameters);
    ...
    if (!isset($options['skip_children'])) {
        ...
        $unpaid_child = fq_saas_get_company_invoice_child_invoices($package_invoice, true);
        if ($unpaid_child) {
 ...
 * @return mixed
 */
function fq_saas_get_company_invoice_child_invoices($invoice, $single_row = true, $excluded_invoices = [])
{
    $dbprefix = fq_saas_master_db_prefix();
    $invoice_table = $dbprefix . 'invoices';
    ...
    return $single_row ? fq_saas_raw_query_row($query, [], true) : fq_saas_raw_query($query, [], true);
}

function fq_saas_invoice_is_on_trial(object $invoice)
{
    return isset($invoice->on_trial) && $invoice->on_trial;
}

function fq_saas_get_invoice_payment_endpoint($invoice)
{
 ...
 * @return string
 */
function fq_saas_db($db)
{
    $db = FQ_SAAS_MODULE_NAME . '_db_' . $db;
 ...
 * @return string
 */
function fq_saas_table($table)
{
    $db_prefix = fq_saas_master_db_prefix();

    return $db_prefix . FQ_SAAS_DB_TABLE_PREFIX . '_' . $table;
}
 ...
 * @return string
 */
function fq_saas_column($column)
{
    return FQ_SAAS_DB_TABLE_PREFIX . '_' . $column;
}

/**
 * Extension tables (FQ SAAS add-ons) use physical prefix fq_saas_, separate from legacy fq_saas_table() core SaaS tables.
 *
 * @param string $entity e.g. activity_log, coupons
 */
function fq_saas_extensions_table(string $entity): string
{
    return db_prefix() . 'fq_saas_' . $entity;
}

/**
 * Persist an audit row on the master database and broadcast a generic hook.
 *
 * @param string $event   Short event key (stored in activity_log.event).
 * @param array  $context Serializable payload.
 */
function fq_saas_log(string $event, array $context = []): void
{
    Fq_saas_kernel::dispatch($event, $context);
    ...
    $CI = &get_instance();
    $table = fq_saas_extensions_table('activity_log');
    if (!$CI->db->table_exists($table)) {
        return;
    }

    $staff_id = function_exists('is_staff_logged_in') && is_staff_logged_in() && function_exists('get_staff_user_id')
    ...
    hooks()->do_action('fq_saas_activity_logged', ['event' => $event, 'context' => $context]);
}

/**
 * Copy options from legacy perfex_saas_% keys to fq_saas_% when the target is missing.
 */
function fq_saas_migrate_legacy_module_options(): void
{
    if (fq_saas_is_tenant()) {
        return;
    }
    ...
    $CI->db->like('name', 'perfex_saas_', 'after');
    ...
    foreach ($rows as $row) {
        if (!str_starts_with($row->name, 'perfex_saas_')) {
            continue;
        }
        $new_name = 'fq_saas_' . substr($row->name, strlen('perfex_saas_'));
        $exists    = $CI->db->where('name', $new_name)->count_all_results($table) > 0;
        if ($exists) {
            continue;
        }
        $CI->db->insert($table, [
            'name'     => $new_name,
            'value'    => $row->value,
            'autoload' => $row->autoload ?? 1,
        ]);
    }
}

/**
 * DNS verification stub for custom domains (extend with real DNS + panel API).
 *
 * @return array{ok: bool, message: string}
 */
function fq_saas_domain_dns_probe(string $domain): array
{
    ...
    $result = hooks()->apply_filters('fq_saas_domain_dns_probe', null, ['domain' => $domain]);
    ...
    return [
        'ok'      => false,
        'message' => 'DNS verification is not configured. Implement filter fq_saas_domain_dns_probe or wire cPanel/Plesk in libraries/integrations.',
    ];
}
 ...
 * @return string
 */
function fq_saas_master_tenant_slug()
{
    return 'master';
}
 ...
 * @return     bool
 */
function fq_saas_is_tenant()
{
    return defined('FQ_SAAS_TENANT_BASE_URL') && defined('FQ_SAAS_TENANT_SLUG') && !empty($GLOBALS[FQ_SAAS_MODULE_NAME . '_tenant']);
}
 ...
 */
function fq_saas_tenant()
{
    if (!fq_saas_is_tenant()) return false;

    $tenant = (object)$GLOBALS[FQ_SAAS_MODULE_NAME . '_tenant'];
 ...
 * @return mixed
 */
function fq_saas_tenant_get_super_option($key = '', $tenant = null)
{
    $tenant = empty($tenant) ? fq_saas_tenant() : $tenant;
...
/**
 * Check whether the current tenant is a demo instance and return scheduling info
 * about the upcoming automatic reset.
 *
 * Returns an associative array with:
 *   - is_demo             (bool)
 *   - reset_hour_interval (float|int) hours between resets
 *   - last_reset_time     (int)       unix timestamp of last reset (may be 0)
 *   - next_reset_time     (int)       unix timestamp of next scheduled reset
 *   - seconds_until_reset (int)       seconds remaining until next reset (>=0)
 *
 * Returns null when the context is not a tenant or not a demo instance.
 *
 * @return array|null
 */
function fq_saas_tenant_demo_reset_info()
{
    if (!fq_saas_is_tenant()) return null;
    ...
    $demo_slugs = is_string($demo_raw) ? (array) json_decode($demo_raw) : (array) $demo_raw;
    if (!in_array($slug, $demo_slugs, true)) return null;

    $reset_hours = (float) fq_saas_tenant_get_super_option('fq_saas_demo_instance_reset_hour');
    if ($reset_hours <= 0) return null; // not scheduled

    $last_reset = (int) fq_saas_tenant_get_super_option('fq_saas_demo_instance_last_reset_time');
    if ($last_reset <= 0) {
        // No prior reset recorded -- assume next reset happens after the full interval from now.
        $last_reset = time();
    }

    $next_reset = (int) ($last_reset + ($reset_hours * 3600));
    $seconds_until = max(0, $next_reset - time());

    return [
        'is_demo'             => true,
        'reset_hour_interval' => $reset_hours,
 ...
 * @return bool
 */
function fq_saas_tenant_is_enabled($permission)
{
    $permission = 'fq_saas_enable_' . $permission;
    $options = fq_saas_tenant_get_super_option();
 ...
 * @return     string|false
 */
function fq_saas_tenant_slug()
{
    if (!fq_saas_is_tenant()) return false;

    return defined('FQ_SAAS_TENANT_SLUG') ? FQ_SAAS_TENANT_SLUG : false;
}
 ...
 * @return string
 */
function fq_saas_route_id_prefix($append)
{
    return FQ_SAAS_ROUTE_ID . '_' . $append;
}
 ...
 */
function fq_saas_master_db_prefix()
{
 ...
 * @return string
 */
function fq_saas_tenant_db_prefix($slug, $table = '')
{
    $master_prefix = fq_saas_master_db_prefix();
    $suffix = $master_prefix;
    if (!empty($table)) {
        $suffix = str_starts_with($table, $master_prefix) ? $table : $master_prefix . $table;
    }

    return fq_saas_str_to_valid_table_name($slug) . '_' . $suffix;
}
 ...
 * @throws Exception
 */
function fq_saas_str_to_valid_table_name($input_string)
{
 ...
 * @return string
 */
function fq_saas_clean_slug($slug, $scenario = '')
{
 ...
 */
function fq_saas_raw_query($query, $dsn = [], $return = false, $atomic = true, $callback = null, $disable_foreign_key = false, $stop_on_error = true, $query_params = [])
{

    if (empty($dsn)) {

        $dsn = fq_saas_master_dsn();
    }

    if (is_string($dsn)) { //conn is dsn sting
        $dsn = fq_saas_parse_dsn($dsn);
    }

    // Get PDO
    $pdo = fq_saas_get_pdo_conn($dsn, true);
    ...
    try {
        ...
        foreach ($queries as $index => $q) {
            ...
            if (!$stop_on_error) {
                try {
                    $stmt = fq_saas_pdo_safe_query($pdo, $q, $query_params);
                } catch (\Throwable $th) {
 ...
 */
function fq_saas_pdo_safe_query($pdo, $query, $parameters)
{
 ...
 */
function fq_saas_raw_query_row($query, $dsn = [], $return = false, $atomic = true, $query_params = [])
{
    $result = fq_saas_raw_query($query, $dsn, $return, $atomic, null, false, true, $query_params);
    if (!$result) {
        return $result;
    }
    return $result[0];
 ...
 */
function fq_saas_db_query($sql)
{
    $slug = fq_saas_tenant_slug();

    if (!$slug) {
        // Default saas panel.
        if (
            stripos($sql, fq_saas_table('')) !== false //saas table queries
        )
            return $sql;

        // Always set this for security. This ensure other tenant data is not loaded for master instance on multitenant singledb
        $slug = fq_saas_master_tenant_slug();
    }
    return fq_saas_simple_query($slug, $sql);
}
 ...
 */
function fq_saas_simple_query($slug, $sql)
{
    $is_master = $slug === fq_saas_master_tenant_slug();
    if ($is_master) return $sql;

    $master_dbprefix = fq_saas_master_db_prefix();
    ...
    if ($will_change_db_struct) {
        ...
        $query_table_names = [];
        fq_saas_extract_dangerous_query_table_name($parsed, $query_table_names);
 ...
 * @return array
 */
function fq_saas_extract_dangerous_query_table_name($parsed, &$tableNames)
{
    ...
    foreach ($parsed as $value) {
        ...
        if (is_array($value)) {
            fq_saas_extract_dangerous_query_table_name($value, $tableNames);
        }
    }
}
 ...
 * @return bool
 */
function fq_saas_is_valid_custom_domain($domain)
{
    ...
    if (str_ends_with($domain, fq_saas_get_saas_default_host()))
        return false;
 ...
 */
function fq_saas_dsn_to_string(array $dsn, $with_auth = true)
{
 ...
 */
function fq_saas_parse_dsn($dsn, $returnKeys = [])
{
    ...
    if (empty($driver)) {
        throw new Exception(_l("fq_saas_invalid_dsn_no_driver"));
    }
    ...
    if ($returnSet) {
        ...
        foreach ($indexes as $key) {
            ...
            if (!isset($parsedDsn[$key])) {
                throw new RuntimeException(_l('fq_saas_dsn_missing_key', $key));
            }
 ...
 */
function fq_saas_is_valid_dsn(array $dsn, $use_cache = true)
{
    try {
        // Check if the required DSN components (host, user, dbname) are present
        if (empty($dsn['host'] ?? '') || empty($dsn['user'] ?? '') || empty($dsn['dbname'] ?? '')) {
            throw new \Exception(_l('fq_saas_host__user_and_dbname_is_required_for_valid_dsn'), 1);
        }

        // Test the database connection
        $conn = fq_saas_get_pdo_conn($dsn, $use_cache);
 ...
 */
function fq_saas_get_pdo_conn($dsn, $use_cache = true)
{
    ...
    $dsn_string = fq_saas_dsn_to_string($dsn, false);
 ...
 */
function fq_saas_show_tenant_error($heading, $message, $error_code = 403, $template = '404', $tag = '')
{
    // Apply the filter to determine if the abrupting error should be prevented or not
    if (function_exists('hooks')) {
        $filter = hooks()->apply_filters('fq_saas_show_tenant_error', ['show_error' => true, 'tag' => $tag, 'heading' => $heading, 'message' => $message, 'error_code' => $error_code, 'template' => $template]);
 ...
 */
function fq_saas_tenant_url_signature($slug)
{
    $path_prefix = FQ_SAAS_ROUTE_ID;
 ...
 */
function fq_saas_http_host($server, $use_forwarded_host = true)
{
 ...
 * @return bool
 */
function fq_saas_is_https($server = [])
{
 ...
 */
function fq_saas_url_origin($server, $use_forwarded_host = true, $include_full_uri = false)
{
    $ssl = fq_saas_is_https($server);
    $protocol = $ssl ? 'https' : 'http';
    $port = $server['SERVER_PORT'];
    $port = ((!$ssl && $port == '80') || ($ssl && $port == '443')) ? '' : ':' . $port;
    $host = fq_saas_http_host($server, $use_forwarded_host);
    $host = isset($host) ? $host : $server['SERVER_NAME'] . $port;

    $folder = parse_url(fq_saas_default_base_url(), PHP_URL_PATH);
 ...
 */
function fq_saas_redirect_back()
{
    ...
    header('Location: ' . fq_saas_url_origin($_SERVER));
    exit();
}
 ...
 */
function fq_saas_http_request($url, $options)
{
    // Initialize cURL
    $curl = curl_init($url);
 ...
 * @param bool        $include_registered_global_fq_saas_modules  Whether to include FQ SAAS specific modules registered as global (cross-contex)
...
function fq_saas_tenant_modules(
    ...
    if ($include_saas_module) {
        $tenant_modules[] = FQ_SAAS_MODULE_NAME;
    }
    ...
    if ($include_registered_global_fq_saas_modules) {
        $tenant_modules = array_merge($tenant_modules, fq_saas_registered_extensions($tenant));
    }
    ...
    if (!$include_tenant_local_disabled_modules && fq_saas_is_tenant()) {
 ...
 */
function fq_saas_registered_extensions(?object $tenant = null, $global_only = true)
{
    // Get the tenant object
    $tenant = $tenant ?? fq_saas_tenant();

    $fq_saas_global_active_modules = [];

    if ($global_only) {
        try {
            $global_active_modules = fq_saas_tenant_get_super_option(FQ_SAAS_GLOBAL_ACTIVE_MODULES_OPTION_KEY, $tenant);

            if (!empty($global_active_modules)) {
                $fq_saas_global_active_modules =  (array)json_decode($global_active_modules);
            }
        } catch (\Throwable $th) {
    ...
    return $fq_saas_global_active_modules;
}
 ...
 */
function fq_saas_tenant_disabled_default_modules(?object $tenant = null, $mode = "controller")
{
    static $cache = [];
    ...
    if (function_exists('hooks')) {
        $tenant_default_disabled_modules  = hooks()->apply_filters('fq_saas_tenant_disabled_default_modules_list_filter', $tenant_default_disabled_modules, $tenant, $mode);
        $can_cache = true;
    }
 ...
 */
function fq_saas_alias_disabled_default_modules($default_disabled_modules)
{
 ...
 * @Bridge-Function
 */
function fq_saas_get_options($field, bool $parse = true)
{
    $single_option = is_string($field);
    $fields = "'" . implode("','", $single_option ? [$field] : $field) . "'";
    $option_query = 'SELECT name, value FROM ' . fq_saas_master_db_prefix() . "options WHERE `name` IN ($fields)";

    // Perform a raw query to fetch the shared secret master settings
    $result = fq_saas_raw_query($option_query, [], true);
 ...
 * @Bridge-Function
 */
function fq_saas_update_option($field, $value)
{
    $option_query = 'UPDATE `' . fq_saas_master_db_prefix() . "options` SET `value` ='$value' WHERE `name`='$field'";
    return fq_saas_raw_query($option_query, []);
}
 ...
 * @Bridge-Function
 */
function fq_saas_get_or_save_client_metadata($clientid, $update_data = [])
{
    $table = fq_saas_table('client_metadata');
    $client_metadata = fq_saas_raw_query_row("SELECT * FROM $table WHERE `clientid`='$clientid';", [], true);
    ...
    $updated = fq_saas_raw_query($query);
    if ($updated)
        return $metadata;
 ...
 * @return array|null
 */
function fq_saas_search_client_metadata($field, $value, $is_unsigned = true, $clientid = null)
{
    $table = fq_saas_table('client_metadata');
    ...
    $client_metadata = fq_saas_raw_query_row($query, [], true, false, $params);
    $metadata = empty($client_metadata->metadata) ? [] : (array)json_decode($client_metadata->metadata);
 ...
 * @return string
 */
function fq_saas_get_system_update_url($purchase_code, $action = 1, $module = '')
{
    return str_replace(['[PC]', '[AC]', '[MD]'], [$purchase_code, $action, $module], FQ_SAAS_UPDATE_URL);
}