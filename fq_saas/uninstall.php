...
require_once(__DIR__ . '/helpers/fq_saas_setup_helper.php');
 ...
 * @return void
 */
function fq_saas_remove_installation($remove_data = false)
{
    $CI = &get_instance();

    // Run common uninstall with data
    fq_saas_uninstall($remove_data);

    if ($remove_data === true) {
        // Remove Perfex SAAS options from the database
        try {
            $CI->db->query("DELETE FROM `" . db_prefix() . 'options' . "` WHERE `name` LIKE 'perfex_saas%' OR `name` LIKE 'fq_saas%'");
        } catch (\Throwable $th) {
        }

        // Drop SAAS tables
        if ($CI->db->table_exists(fq_saas_table('packages'))) {
            $CI->db->query("DROP TABLE `" . fq_saas_table('packages') . "`;");
        }
        if ($CI->db->table_exists(fq_saas_table('companies'))) {
            $CI->db->query("DROP TABLE `" . fq_saas_table('companies') . "`;");
        }
        if ($CI->db->field_exists(fq_saas_column('packageid'), db_prefix() . 'invoices')) {
            $CI->db->query("ALTER TABLE `" . db_prefix() . "invoices` DROP `" . fq_saas_column('packageid') . "`;");
        }
...
if (!empty($option)) {
    ...
    if ($option == 'uninstall_no_data') {
        $confirmed = true;
        fq_saas_remove_installation(false);
    }
    ...
    if ($option == 'uninstall_with_data') {
        ...
        if ($password === $confirm_text) {
            $confirmed = true;
            fq_saas_remove_installation(true);
        }
    }
}
