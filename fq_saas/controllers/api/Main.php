...
trait Main
{
    ...
    public function caddy_domain_check()
    {
        ...
        if (fq_saas_get_saas_default_host() === $domain) {
        ...
        $tenant_info = fq_saas_get_tenant_info_by_host($domain);
        if ($tenant_info) {
            ...
            $tenant = fq_saas_search_tenant_by_field($field, $value);
            if ($tenant) {
                set_status_header(200);
                echo "Matched";
                return;
    ...
    public function is_slug_available($slug)
    {
        ...
        if ($is_available) {
            $validated_slug = fq_saas_generate_unique_slug($slug, 'companies');
            $is_available = $validated_slug === $slug ||
                $validated_slug === fq_saas_clean_slug($slug); // allow dash in slug
        }
    ...
    public function is_custom_domain_available($domain, $client_id = '')
    {
        ...
        if (fq_saas_is_valid_custom_domain($domain)) {
            $this->fq_saas_model->db->group_start();
            $this->fq_saas_model->db->where('custom_domain', $domain, true);
            $this->fq_saas_model->db->or_like('metadata', '"pending_custom_domain":"' . $domain . '"', true);
            $this->fq_saas_model->db->or_like('metadata', "'pending_custom_domain':'" . $domain . "'", true);
            $this->fq_saas_model->db->group_end();
            ...
            if ($client_id) {
                $this->fq_saas_model->db->group_start()->where('`clientid` !=', $client_id)->group_end();
            }
            $_companies = $this->fq_saas_model->get(fq_saas_table('companies'));
     ...
     *             example={
     *                 "fq_saas_enable_single_package_mode": "0",
     *                 "fq_saas_enable_auto_trial": "0",
     *                 "fq_saas_require_invoice_payment_status": "[""4""]"
     *             }
     *        )
     *     ),
    ...
    public function settings($group = '')
    {
        ...
        $keys = [
            ...
            "fq_saas_enable_auto_trial",
            "fq_saas_autocreate_first_company",
            "fq_saas_allow_customer_cancel_subscription",
            "fq_saas_require_invoice_payment_status",
            "fq_saas_autolaunch_instance",
            "fq_saas_alternative_base_host",
            "fq_saas_route_id",
            "fq_saas_reserved_slugs",
            "fq_saas_enable_subdomain_input_on_signup_form",
            "fq_saas_enable_customdomain_input_on_signup_form",
            "fq_saas_landing_page_url",
            "fq_saas_landing_page_url_mode",
            "fq_saas_clients_default_theme_whitelabel_name",
            "fq_saas_instance_delete_pending_days",
            "fq_saas_tenant_seeding_source",
            "fq_saas_seeding_tenant",
            "fq_saas_enable_custom_module_request",
        ];
    ...
    public function modules()
    {
        ...
        $modules = $this->fq_saas_model->modules();
        $result = [];
    ...
    public function services()
    {
        ...
        $services = $this->fq_saas_model->services();
        $result = [];
    ...
    public function plans($id = '')
    {
        ...
        $_packages = $this->fq_saas_model->packages($id);
     ...
     * @OA\Get(
     *     tags={"CMS"},
     *     path="/saas/api/cms_pages",
     *     summary="List published CMS pages (read-only)",
     *     @OA\Parameter(
     *         name="type",
     *         in="query",
     *         required=false,
     *         @OA\Schema(type="string"),
     *         description="Optional type filter: page, blog, faq"
     *     ),
     *     @OA\Response(
     *         response=200,
    ...
    public function cms_pages()
    {
        ...
        $rows = $this->fq_saas_extensions_model->cms_pages((string) $type);
        $out = [];
        foreach ($rows as $r) {
        ...
        return $this->response_json(['pages' => $out]);
    }
    ...
    public function tenants($id = '', $client_id = '')
    {
        ...
        try {
            ...
            $this->fq_saas_model->db->limit($limit);
            $this->fq_saas_model->db->offset($offset);
            if ($client_id)
                $this->fq_saas_model->db->where('clientid', $client_id);
            $_tenants = $this->fq_saas_model->companies($id);
            ...
            foreach ($_tenants as $key => $tenant) {
                $tenant_list[] = [
                    ...
                    "dsn" => fq_saas_dsn_to_string(fq_saas_parse_dsn($tenant->dsn, ['host', 'dbname']), false),
                    "accessible_url_list" => fq_saas_tenant_base_url($tenant, '', 'all'),
    ...
    public function delete_tenant($tenant_id)
    {
        ...
        try {
            $id = (int)$tenant_id;
            if ($id) {
                $removed = $this->fq_saas_model->delete_company($id);
                if ($removed) {
                    $message = _l('deleted', _l('fq_saas_company')) . ($removed !== true ? ' ' . _l('fq_saas_with_error') . ': ' . $removed : '');
                    return $this->response_json(['success' => $message]);
                }
                $message =  _l('fq_saas_error_completing_action') . (is_string($removed) ? ': ' . $removed : '');
                throw new \Exception($message, 1);
            }

            throw new \Exception(_l('fq_saas_error_completing_action'));
        } catch (\Throwable $th) {
            return $this->response_json(['error' => $th->getMessage()], 500);
        }
    }
    ...
    public function cancel_subscription($clientid)
    {
        ...
        if (!(int)get_option('fq_saas_allow_customer_cancel_subscription')) {
            return $this->response_json(['error' => _l('fq_saas_allow_customer_cancel_subscription')], 423);
        }
    ...
    private function api_key_middleware($method = 'GET')
    {
        ...
        if (empty($apiMethod) || empty($method)) {
            return $this->response_json(['error' => 'TRACE_ERROR' . _l('fq_saas_permission_denied')], 405);
        }
        ...
        if ($this->input->server('REQUEST_METHOD', true) !== strtoupper($method)) {
            return $this->response_json(['error' => _l('fq_saas_invalid_method')], 405);
        }
    ...
    {
        if (get_option('fq_saas_enable_api') != 1) {
            return $this->response_json(['error' => _l('fq_saas_api_not_enabled')], 401);
        }
        return true;
    }
