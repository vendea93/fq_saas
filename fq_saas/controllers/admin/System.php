...
class System extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->returnUrl = admin_url(FQ_SAAS_ROUTE_NAME . '/system');
    }
    ...
    public function index()
    {
        ...
        $data['title'] = _l('fq_saas_update_ext');
        $data['purchase_code'] = $purchase_code;
        $data['saas_module'] = $this->app_modules->get(FQ_SAAS_MODULE_NAME);
        ...
        $data['remote_modules'] = (object)hooks()->apply_filters('fq_saas_system_remote_extensions', $data['remote_modules']);
        ...
        if ($this->input->post()) {
            ...
            try {
                ...
                if (empty($purchase_code))
                    throw new \Exception(_l("fq_saas_invalid_purchase_code"), 1);

                $purchase_code = trim($purchase_code);

                // Validate code on server
                $url = fq_saas_get_system_update_url($purchase_code);
                $request = (object)fq_saas_http_request($url, []);
                if (!empty($request->error))
                ...
                if (empty($purchase) || empty($purchase->buyer)) {
                    ...
                    throw new \Exception(_l("fq_saas_error_fetching_purchase_code_details"), 1);
                }

                // Save purchase code in cache
                update_option('fq_saas_purchase_code', $purchase_code);

                $status = 'success';
                $message = empty($response->message) ? _l('updated_successfully', _l('fq_saas_purchase_code')) : $response->message;
            } catch (\Throwable $th) {
    ...
    public function get_module($module)
    {
        ...
        try {
            ...
            // Usage
            $defaultRemoteFileUrl = fq_saas_get_system_update_url($purchase_code, 3, $module); // Replace with the actual remote file URL

            // Allow customizing the module source url.
            $remoteFileUrl = hooks()->apply_filters('fq_saas_system_remote_extension_url', ['url' => $defaultRemoteFileUrl, 'purchase_code' => $purchase_code, 'module' => $module]);
            ...
            if ($responseCode === 200 && strpos($contentType, 'application/zip') !== false) {
                ...
                try {
                    ...
                    if ($this->app_module_installer->check_module($moduleTemporaryDir) === false) {
                        $response['message'] = _l('fq_saas_module_not_found');
                    } else {
                    ...
                    fq_saas_remove_dir($moduleTemporaryDir);

                    $response['message'] = _l('fq_saas_module_installed_successfully');
                } catch (Exception $e) {
                    $response['message'] = $e->getMessage();
                }
            } else {
                // It's not a file download response, directly output JSON text
                $_response = fq_saas_http_request($remoteFileUrl, []);
        ...
        return redirect(admin_url(FQ_SAAS_ROUTE_NAME . '/system/after_get_module/' . $module) . "?status=$status&message=$message");
    }
    ...
    public function after_get_module($module)
    {
        ...
        try {
            if ($status !== 'danger') {
                ...
                $this->fq_saas_cron_model->check_saas_latest_version();
            }
            set_alert($status, $message);
        } catch (\Throwable $th) {
