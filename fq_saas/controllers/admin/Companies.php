...
class Companies extends AdminController
{
    ...
    function index()
    {
        ...
        if (!staff_can('view', 'fq_saas_companies')) {
            return access_denied('fq_saas_companies');
        }

        // Return the table data for ajax request
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data(module_views_path(FQ_SAAS_MODULE_NAME, 'companies/table'));
        }

        // Show list of comapnies
        $data['companies'] = $this->fq_saas_model->companies();
        $data['title'] = _l('fq_saas_companies');
    ...
    function create()
    {
        ...
        if (!staff_can('create', 'fq_saas_companies')) {
            return access_denied('fq_saas_companies');
        }
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('name', _l('fq_saas_name'), 'required');
            $this->form_validation->set_rules('clientid',  _l('fq_saas_customer'), 'required');
            $this->form_validation->set_rules('db_scheme', _l('fq_saas_db_scheme'), 'required');
            ...
            if ($this->form_validation->run() !== false) {
                ...
                try {
                    ...
                    $invoice  = $this->fq_saas_model->get_company_invoice($form_data['clientid']);
                    ...
                    if (!isset($invoice->db_scheme) || empty($invoice->db_scheme)) {
                        throw new \Exception(_l('fq_saas_no_invoice_client'), 1);
                    }
                    ...
                    if ($db_scheme == 'shard') {
                        ...
                        $validation = fq_saas_is_valid_dsn($form_data['db_pools']);
                        $dsn_string = fq_saas_dsn_to_string($form_data['db_pools']);
                    ...
                    $_id = $this->fq_saas_model->create_or_update_company($form_data, $invoice);
                    if ($_id) {

                        set_alert('success', _l('added_successfully', _l('fq_saas_company')));
                        return redirect(admin_url(FQ_SAAS_ROUTE_NAME . '/companies'));
                    }

                    // Log error
                    log_message('error', _l('fq_saas_error_completing_action') . ':' . ($this->db->error() ?? ''));

                    throw new \Exception(_l('fq_saas_error_completing_action'), 1);
                } catch (\Exception $e) {

                    set_alert('danger', $e->getMessage());
                    return fq_saas_redirect_back();
                }
            }
        }
    ...
    function edit($id)
    {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                try {
                    ...
                    if ($_id) {
                        ...
                        set_alert('success', _l('updated_successfully', _l('fq_saas_company')));
                        return redirect(admin_url(FQ_SAAS_ROUTE_NAME . '/companies'));
                    }
        ...
        $data['company'] = $this->fq_saas_model->companies($id);
        $data['title'] = _l('fq_saas_companies');
        $this->load->view('companies/form', $data);
    }
    ...
    function custom_domain()
    {
        ...
        if ($this->input->post()) {
            try {
                ...
                $form_data = $this->input->post(NULL, true);
                $id = $form_data['id'];
                $company = $this->fq_saas_model->companies($id);
                ...
                // Get the client invoice
                $invoice  = $this->fq_saas_model->get_company_invoice($company->clientid);

                // Save and make deployment another job
                $_id = $this->fq_saas_model->create_or_update_company($data, $invoice);
                if ($_id) {

                    fq_saas_send_customdomain_request_notice($company, $custom_domain, $invoice, $approve ? 'approved' : 'rejected', ['extra_note' => $form_data['extra_note']]);

                    if ($approve) {
                        fq_saas_log('domain_verified', [
                            'company_id'    => (int) $company->id,
                            'custom_domain' => (string) $custom_domain,
                        ]);
                    }

                    set_alert('success', _l('updated_successfully', _l('fq_saas_custom_domain')));
                    return redirect(admin_url(FQ_SAAS_ROUTE_NAME . '/companies/edit/' . $_id));
                }
    ...
    function delete($id)
    {
        if (!staff_can('delete', 'fq_saas_companies')) {
            return access_denied('fq_saas_companies');
        }
        ...
        if ($removed)
            set_alert('success', _l('deleted', _l('fq_saas_company')) . ($removed !== true ? ' ' . _l('fq_saas_with_error') . ': ' . $removed : ''));
        else
            set_alert('danger', _l('fq_saas_error_completing_action') . (is_string($removed) ? $removed : ''));
    ...
    public function update_dsn()
    {
        ...
        try {
            ...
            if (empty($id) || empty($dsn)) throw new \Exception(_l('fq_saas_error_completing_action'), 1);

            $data = ['id' => $id];

            // Validate the provided custom db credentails
            $validation = fq_saas_is_valid_dsn($dsn);
            $dsn_string = fq_saas_dsn_to_string($dsn);
    ...
    {
        if (fq_saas_is_tenant() && is_admin()) {
            $tenant = fq_saas_tenant();
            $clientid = $tenant->clientid;
            if ($clientid) {
                $auth_code = fq_saas_generate_magic_auth_code($clientid);
                ...
                if ($auth_code) {
                    ...
                    $url = fq_saas_default_base_url($query . '&redirect=' . $redirect . '&source_url=' . current_url());

                    // When cross domain not allow, and tenant using custom domain, open in new tab instead.
                    $support_custom_domain_magic_login = fq_saas_tenant_is_enabled('cross_domain_bridge');
                    if ($tenant->http_identification_type === FQ_SAAS_TENANT_MODE_DOMAIN && !$support_custom_domain_magic_login) {
