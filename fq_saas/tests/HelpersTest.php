...
class HelpersTest extends FqSaasTest
{

    /**
     * Method to test fq_saas_get_tenant_info_by_request_uri helper function.
     * fq_saas_get_tenant_info_by_request_uri is used for detecting the tenant from request uri.
     *
     * @return void
     */
    public function testFqSaasGetTenantInfoByRequestUri()
    {
        $testCases = [
            [
        ...
        foreach ($testCases as $testCase) {
            $result = fq_saas_get_tenant_info_by_request_uri($testCase['request_uri']);
     ...
     * @return void
     */
    public function testFqSaasGetTenantInfoByHost()
    {
        $defaultHost = fq_saas_get_saas_default_host();
        $testCases = [
            // Host with Subdomain
            [
                'http_host' => 'www.tenant1.' . fq_saas_get_saas_default_host(),
                'expected_result' => [
                    'custom_domain' => '',
                    'slug' => 'tenant1',
                ],
            ...
            [
                'http_host' => 'www.' . fq_saas_get_saas_default_host(),
                'expected_result' => false,
                'exception' => false,
                'exception_class' => '',
        ...
        foreach ($testCases as $testCase) {
            ...
            try {
                $result = fq_saas_get_tenant_info_by_host($testCase['http_host']);
                $this->CI->unit->run(
                    $result,
                    $testCase['expected_result'],
                    $testCase['description'],
     ...
     * @return void
     */
    public function testFqSaasGetTenantInfoByHttp()
    {
        $defaultHost = fq_saas_get_saas_default_host();
        ...
        foreach ($testCases as $testCase) {
            ...
            try {
                $result = fq_saas_get_tenant_info_by_http($testCase['request_uri'], $testCase['host']);
                $this->CI->unit->run(
                    $result,
                    $testCase['expected_result'],
                    $testCase['description'],
    ...
    /**
     * Test the fq_saas_simple_query helper.
     * 
     * The helper parse query and localize to the active tenant.
     *
     * @return void
     */
    public function testFqSaasSimpleQuery()
    {
        $slug = 'tenant_slug';
        ...
        $valid_queries = [
            [
                ...
                'expected_result' => 'SELECT * FROM customers WHERE customers.' . FQ_SAAS_TENANT_COLUMN . ' = \'tenant_slug\'',
                'test_name' => 'Valid Query Test',
            ],
            [
                'sql' => 'SELECT COUNT(*) FROM orders WHERE status = "completed"',
                'expected_result' => 'SELECT COUNT(*) FROM orders WHERE status = "completed" and orders.' . FQ_SAAS_TENANT_COLUMN . ' = \'tenant_slug\'',
                'test_name' => 'Valid Query with Where Clause Test',
            ],
            [
                'sql' => 'SELECT * FROM products WHERE price > 100',
                'expected_result' => 'SELECT * FROM products WHERE price > 100 and products.' . FQ_SAAS_TENANT_COLUMN . ' = \'tenant_slug\'',
                'test_name' => 'Valid Query with Where Clause and Comparison Test',
            ],
        ];

        foreach ($valid_queries as $query) {
            $result = fq_saas_simple_query($slug, $query['sql']);
        ...
        foreach ($invalid_queries as $query) {
            ...
            try {
                fq_saas_simple_query($slug, $query['sql']);
        ...
        $write_queries = [
            [
                ...
                'expected_result' => "INSERT INTO orders (customer_id, total, `" . FQ_SAAS_TENANT_COLUMN . "`) VALUES (1, 100, 'tenant_slug')",
                'test_name' => 'Insert Query Test',
            ],
            [
                'sql' => 'UPDATE customers SET name = "John Doe" WHERE id = 1',
                'expected_result' => 'UPDATE customers SET name = "John Doe",`' . FQ_SAAS_TENANT_COLUMN . '` = \'tenant_slug\' WHERE id = 1 and customers.' . FQ_SAAS_TENANT_COLUMN . ' = \'tenant_slug\'',
                'test_name' => 'Update Query Test',
            ],
            [
                'sql' => 'DELETE FROM products WHERE price < 10',
                'expected_result' => 'DELETE FROM products WHERE price < 10 and products.' . FQ_SAAS_TENANT_COLUMN . ' = \'tenant_slug\'',
                'test_name' => 'Delete Query Test',
            ],
            [
                'sql' => 'INSERT INTO orders (customer_id, total,' . FQ_SAAS_TENANT_COLUMN . ') VALUES (1, 100,"tenant_slug")',
                'expected_result' => "INSERT INTO orders (customer_id, total, `" . FQ_SAAS_TENANT_COLUMN . "`) VALUES (1, 100, 'tenant_slug')",
    ...
    /**
     * Test fq_saas_parse_dsn helper for parsing DSN string to array
     *
     * @return void
     */
    public function testFqSaasParseDsn()
    {
        // Valid DSN
        ...
        $valid_result = fq_saas_parse_dsn($valid_dsn);
        $this->CI->unit->run(
            $valid_result,
            $valid_expected_result,
            'Valid DSN Test',
        ...
        $valid_result_specific_keys = fq_saas_parse_dsn($valid_dsn_specific_keys, ['driver', 'dbname']);
        $this->CI->unit->run(
        ...
        try {
            fq_saas_parse_dsn($invalid_dsn_no_driver);
    ...
    /**
     * Test for fq_saas_safe_query helper for importing perfex sql file
     *
     * @return void
     */
    public function testFqSaasSafeQuery()
    {
        // Safe query
        $safe_query = 'SELECT * FROM customers';
        $safe_expected_result = $safe_query;

        $safe_result = fq_saas_safe_query($safe_query, 'current_database');
        $this->CI->unit->run(
            $safe_result,
        ...
        try {
            fq_saas_safe_query($empty_query, 'current_database');
        ...
        try {
            fq_saas_safe_query($prohibited_query, 'current_database');
        ...
        try {
            fq_saas_safe_query($injection_query, 'current_database');
        ...
        try {
            fq_saas_safe_query($dangerous_query, 'current_database');
