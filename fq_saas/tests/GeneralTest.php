 ...
 */
class GeneralTest extends FqSaasTest
{
    ...
    public function testInstanceDeploySchemes()
    {
        ...
        $t_package = $this->CI->fq_saas_model->packages()[0];
        $t_company = $this->CI->fq_saas_model->companies()[0];
        ...
        $dsn_string = fq_saas_dsn_to_string(['dbname' => 'test']);
        $test = fq_saas_is_valid_dsn($default_dsn);
        ...
        $dsn_string = fq_saas_dsn_to_string(['dbname' => 'test']);
        $test = fq_saas_parse_dsn($dsn_string);
        $expected_result = [
            "driver" => APP_DB_DRIVER,
            "host" => APP_DB_HOSTNAME,
            "dbname" => 'test',
        ...
        // Perfex table column 
        $test = fq_saas_column('packageid');
        $expected_result = FQ_SAAS_MODULE_NAME . '_packageid';
        $test_name = 'should prefix perfex table column with proper prefix';
        $this->CI->unit->run($test, $expected_result, $test_name, $this->customTestNote($test, $expected_result));

        // Saas table prefix
        $test = fq_saas_table('company');
        $expected_result = db_prefix() . FQ_SAAS_MODULE_NAME . '_company';
        $test_name = 'should prefix saas table with proper prefix';
        $this->CI->unit->run($test, $expected_result, $test_name, $this->customTestNote($test, $expected_result));


        // Saas database prefix
        $test = fq_saas_db('company');
        $expected_result = FQ_SAAS_MODULE_NAME . '_db_company';
        $test_name = 'should generate right instance database name';
        $this->CI->unit->run($test, $expected_result, $test_name, $this->customTestNote($test, $expected_result));


        // Parse dsn to string
        $test = fq_saas_dsn_to_string(['dbname' => 'demo']);
        ...
        $test = fq_saas_generate_unique_slug($t_company->slug, 'companies');
        ...
        $test = fq_saas_generate_unique_slug($t_company->slug, 'companies', $t_company->id);
        ...
        $test = fq_saas_is_valid_dsn($default_dsn, false);
        $expected_result =  true;
        $test_name = 'should validate dsn connection';
        $this->CI->unit->run($test, $expected_result, $test_name, $this->customTestNote($test, $expected_result));

        // Ensure throw message on invalid dsn connection
        $test = fq_saas_is_valid_dsn(array_merge($default_dsn, ['password' => time()]), false);
        ...
        $test = fq_saas_get_company_dsn($company);
        ...
        $company->dsn = APP_DB_DRIVER . ':host=localhost;dbname=fq_saas_db_tenant_ulutfa;user=;password=;';
        $test = fq_saas_get_company_dsn($company);
        $expected_result = [
            "driver" => APP_DB_DRIVER,
            "host" => APP_DB_HOSTNAME,
            "dbname" => "fq_saas_db_tenant_ulutfa",
            "user" => APP_DB_USERNAME,
            "password" => ""
        ];
        if (is_array($test))
        ...
        $c = fq_saas_column('packageid');
        $invoice = (object) ['db_scheme' => 'single', "$c" => $t_package->id];
        $test = fq_saas_get_company_dsn($company, $invoice);

        $expected_result = array_merge($default_dsn, ['dbname' => fq_saas_db($company->slug)]);
        if (is_array($test))
            ksort($test);
        if (is_array($expected_result))
            ksort($expected_result);
        ...
        $company->dsn = APP_DB_DRIVER . ':host=localhost;dbname=fq_saas_db_tenant_ulutfa;user=;password=;';
        $this->CI->fq_saas_model->create_database(fq_saas_db($company->slug));
        $test = fq_saas_deploy_company($company);
        $expected_result = true;
        $test_name = 'deploy an instance';
        $this->CI->unit->run($test, $expected_result, $test_name, $this->customTestNote($test, $expected_result));

        // Test instance removal
        $test = fq_saas_remove_company($company);
        $expected_result = true;
        ...
        $expected_result = true;
        $test = send_mail_template('customer_deployed_instance', FQ_SAAS_MODULE_NAME, get_staff(1)->email, $company->clientid, get_contact_user_id($company->clientid), $company);
        $test_name = 'send email template for deployed instance';
        $this->CI->unit->run($test, $expected_result, $test_name, $this->customTestNote($test, $expected_result));

        $expected_result = true;
        $test = send_mail_template('customer_deployed_instance_for_admin', FQ_SAAS_MODULE_NAME, get_staff(1)->email, $company->clientid, get_contact_user_id($company->clientid), $company);
        $test_name = 'send email template for deployed instance to admin';
        $this->CI->unit->run($test, $expected_result, $test_name, $this->customTestNote($test, $expected_result));

        $expected_result = true;
        $test = send_mail_template('customer_removed_instance', FQ_SAAS_MODULE_NAME, get_staff(1)->email, $company->clientid, get_contact_user_id($company->clientid), $company);
        $test_name = 'send email template for removed instance';
        $this->CI->unit->run($test, $expected_result, $test_name, $this->customTestNote($test, $expected_result));

        $expected_result = true;
        $test = send_mail_template('customer_removed_instance_for_admin', FQ_SAAS_MODULE_NAME, get_staff(1)->email, $company->clientid, get_contact_user_id($company->clientid), $company);
