...
function saas_sidebar_menu_items($items): array
{
    if (!empty(subdomain())) {
        ...
        hooks()->add_action('app_admin_footer', function () use ($allUses, $CI) {
            if (in_array($CI->router->fetch_class(), ['dashboard', 'staff', 'profile'])) {
                echo '<script>
                    const DISABLED_FEATURES = ' . json_encode($allUses) . ';
...
function saas_init_menu_items()
{
    ...
    if (!empty(is_admin()) && !empty(subdomain()) || !empty(is_admin()) && !empty($db_name)) {
        ...
        if (ConfigItems('saas_billings_menu') != 'admin_panel') {
            $companyname = ConfigItems('saas_companyname') ?? 'saas';
            $CI->app_menu->add_setup_menu_item('perfect_saas', [
                'name' => '<span class="text-danger">' . $companyname . ' ' . _l('billings') . '</span>',
                'position' => 80,
//                'icon' => 'fa-solid fa-receipt menu-icon text-danger',
                'badge' => [],
            ]);
            $CI->app_menu->add_setup_children_item('perfect_saas', [
                'slug' => 'dashboard',
                'name' => _l('dashboard'),
