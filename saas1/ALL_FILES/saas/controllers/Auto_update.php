...
class Auto_update extends AdminController
{
    ...
    public function index()
    {
        ...
        $itemID = SAAS_ITEM_ID;
        $directory = module_dir_path($module);
        ...
        $url = SAAS_UPDATE_URL . 'api/update_version';
    ...
    protected function copyUpgrade($zipFile)
    {
        ...
        if (@copy($zipFile, $copyFileLocation)) {
            ...
            update_option('saas_update_data', json_encode($optionData));
            $upgradeCopied = true;
        }
    ...
    {
        $lastUpgradeCopyData = get_option('saas_update_data');
    ...
    function database()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            echo '<div class="bold" >
            <h4 class="bold" > Hi!Thanks for updating Perfect SaaS - Powerful Multi-Tenancy Module for Perfex CRM - You are using version ' . $this->latest_version . ' </h4 >
            <p >
        ...
        $result = $this->app_modules->upgrade_database(SaaS_MODULE);
        ...
        set_alert('success', lang('using_latest_version'));
        exit();
    }
