...
//                ),
    ...
    public function active_modules($modules, $companies_history_id): bool
    {
        ...
        if (!empty($companyInfo->db_name)) {
            ...
            // check if saas module
            $saas_module = $this->new_db->where('module_name', 'saas')->get(db_prefix() . 'modules')->row();
            // if not exist then insert it
            if (empty($saas_module)) {
                $installed_version = array_column(array_filter($all_modules, function ($all_module) use ($new_module) {
                    return $all_module['system_name'] == 'saas';
                }), 'headers')[0]['version'];

                $this->new_db->insert(db_prefix() . 'modules', ['module_name' => 'saas', 'installed_version' => $installed_version, 'active' => 1]);
            }
    ...
    public function force_upload_theme($themeBaseDir, $baseDir, $themes = null)
    {
        list($baseThemePath, $baseThemeUrl) = get_theme_path_url();
        try {
            if (!is_dir($themeBaseDir)) {
                mkdir($themeBaseDir, 0755, true);
                if (!empty($themes)) {
                    foreach ($themes as $theme) {
                        if (!is_dir($themeBaseDir . '/' . $theme)) {
                            // check if theme exist in $baseThemePath if not then copy from default_themes
                            if (!is_dir($baseThemePath . '/' . $theme)) {
                                xcopy($baseDir . 'default_themes/' . $theme, $themeBaseDir . '/' . $theme);
                            } else {
                                xcopy($baseThemePath . '/' . $theme, $themeBaseDir . '/' . $theme);
                            }
                        }
                    }
                } else {
                    xcopy($baseDir . 'default_themes', $themeBaseDir);
                }
            } else {
    ...
    function get_update_info($data = array())
    {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_TIMEOUT => 30,
            CURLOPT_URL => SAAS_UPDATE_URL . 'api/latest_version',
            CURLOPT_POST => 1,
    ...
    {
        $version = get_row(db_prefix() . 'modules', array('module_name' => 'saas'), 'installed_version'); // 1.2.1
        $version = str_replace(".", "", $version);
        return $version;

    }

    public function update_version_in_all_company_database()
    {
        $module = $this->app_modules->get(SaaS_MODULE);
        $module_version = $module['headers']['version'];
